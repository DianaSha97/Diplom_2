...
import ru.educationservices.stellarburgers.responseEntities.Ingredient;
import ru.educationservices.stellarburgers.responseEntities.IngredientsResponse;
...
public class CreateOrderTests {
    ...
    public void createOrderWithAuthAndIngredientsIsSuccess() {
        Response response = orderApi.createOrder(
                List.of(ingredient.get(0).get_id(), ingredient.get(ingredient.size() - 1).get_id()),
                token
        );
    ...
    public void createOrderWithAuthAndIncorrectIngredientsIsFailed() {
        Response response = orderApi.createOrder(
                List.of(ingredient.get(0).get_id(), UUID.randomUUID().toString()),
                token
        );
